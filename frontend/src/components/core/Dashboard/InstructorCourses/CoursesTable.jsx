...
// // ✅ Helper: converts total seconds → "Xhr Ymin" or "Ymin" etc.
...
// // ✅ Helper: sums up timeDuration from all subsections across all sections
// const getCourseDuration = (course) => {
//   let totalSeconds = 0

//   course?.courseContent?.forEach((section) => {
//     section?.subSection?.forEach((subSec) => {
//       // timeDuration is usually stored as a string like "120.5" (seconds)
//       const duration = parseFloat(subSec?.timeDuration) || 0
...
//   const [confirmationModal, setConfirmationModal] = useState(null)
//   const TRUNCATE_LENGTH = 25
// // Temporarily add this in your component to inspect
// console.log("Course Data:", JSON.stringify(courses[0], null, 2))
//   // delete course
//   const handleCourseDelete = async (courseId) => {
//     setLoading(true)
//     const toastId = toast.loading('Deleting...');
...
//                   {/* ✅ Dynamic course duration calculated from subSection timeDuration */}
//                   <Td className="text-sm font-medium text-richblack-100">
...
// ✅ Helper: converts total seconds → "Xhr Ymin"
const formatDuration = (totalSeconds) => {
    if (!totalSeconds || totalSeconds === 0) return "0 min"
    const hrs = Math.floor(totalSeconds / 3600)
    const mins = Math.floor((totalSeconds % 3600) / 60)
...
// ✅ Helper: sum timeDuration from all subSections across all sections
const getCourseDuration = (course) => {
    let totalSeconds = 0
                    ...
                    {!loading && courses?.length === 0 ? (
                        ...
                        courses?.map((course) => (
                            ...
                            >
                                ...
                                {/* ✅ Real duration from subSection timeDuration */}
                                <Td className="text-sm font-medium text-richblack-100">
                                    {getCourseDuration(course)}
                                </Td>
